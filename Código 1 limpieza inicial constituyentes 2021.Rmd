...
sen_2021_partido <- sen_2021 %>% group_by(partido_político) %>%
  summarize(votos_totales = sum(as.numeric(cantidad_votos), na.rm=T)) %>%
  ungroup()
...
primera_presi_2017 <- read_excel("2017_11_Presidencial_Datos_Eleccion.xlsx")
pp_2017 <- primera_presi_2017 %>% select(...2, ...11, ...13)
...
pp_2017_1 <- pp_2017 %>% group_by(primer_apellido) %>% 
  summarize(votos_totales = sum(as.numeric(cantidad_votos), na.rm=T)) %>%
  ungroup()
```
